...
- (NSString *)getFirstGroupContainerPathForBundleId:(NSString *)bundleId appProxy:(id)appProxy {
    ...
    // 方法2：扫描 /var/mobile/Containers/Shared/AppGroup 目录，智能匹配 bundleId
        ...
        for (NSString *dir in subDirs) {
            ...
            if ([fm fileExistsAtPath:metadataPath]) {
                ...
                if (identifier) {
                    ...
                    // 后缀匹配（如 group.com.example.app 匹配 com.example.app）
                    if ([identifierLower hasSuffix:bundleIdLower]) {
                        [groupPathCache setObject:groupDir forKey:bundleId];
                        return groupDir;
                    }
                    // 前缀匹配（如 com.example.app.group 匹配 com.example.app）
                    if ([bundleIdLower hasPrefix:identifierLower] || [identifierLower hasPrefix:bundleIdLower]) {
                        [groupPathCache setObject:groupDir forKey:bundleId];
                        return groupDir;
                    }
                    // 包含匹配（宽松，但避免误判，仅当长度相近时）
                    if ([identifierLower rangeOfString:bundleIdLower].location != NSNotFound &&
                        labs((long)identifierLower.length - (long)bundleIdLower.length) <= 5) {
                        [groupPathCache setObject:groupDir forKey:bundleId];
                        return groupDir;
                    }
                }
...
// 增强版 Filza 跳转：支持包含中文、空格、特殊字符的路径
- (void)openInFilza:(NSString *)path {
    ...
    // 构造 Filza URL：filza:///绝对路径
    // 注意：路径需要编码，但保留斜杠。使用 URLPathAllowedCharacterSet 会将 ':' 编码为 %3A，但路径中很少出现冒号。
    // 更安全的做法：先编码，再手动将 %2F 还原为 /（因为 URLPathAllowedCharacterSet 不会编码 /，但 stringByAddingPercentEncodingWithAllowedCharacters 实际上不会编码 /，所以没问题）
    ...
    // 如果编码后 URL 无效，尝试不编码的原始路径（有些 Filza 版本支持）
    if (!url) {
        urlString = [NSString stringWithFormat:@"filza://%@", path];
    ...
    if (url && [[UIApplication sharedApplication] canOpenURL:url]) {
        ...
        // 最后尝试使用 file:// 协议（部分 Filza 版本可能支持）
        ...
        if ([[UIApplication sharedApplication] canOpenURL:fileURL]) {
            ...
            [self showAlert:@"错误" message:@"未安装Filza文件管理器或不支持该路径格式"];
        }
    }
}
